...
const rows=Array.isArray(input.rows)?input.rows:input.tabs?.[0]?.rows;
if(!Array.isArray(rows)||rows.length<2)throw new Error("Input must contain rows with a header row");
const REQUIRED=["Topics","Thai word","Phonetic word","English word","Example phrase","English example phrase","Date added"];
...
const cards=rows.slice(1).filter(row=>row.some(value=>normalize(value))).map((row,position)=>{
 ...
 const thai=normalize(row[index["Thai word"]]),phonetic=normalize(row[index["Phonetic word"]]),english=normalize(row[index["English word"]]),exampleThai=normalize(row[index["Example phrase"]]),exampleEnglish=normalize(row[index["English example phrase"]]),dateAdded=normalize(row[index["Date added"]]);
 if(![thai,phonetic,english,exampleThai,exampleEnglish,dateAdded].every(Boolean))throw new Error(`Incomplete row ${position+2}`);
 if(!exampleThai.includes(thai))throw new Error(`Example does not contain Thai word at row ${position+2}`);
 ...
 return{id:`card-${identity}-${occurrence}`,topics:cardTopics,thai,phonetic,english,exampleThai,exampleEnglish,dateAdded,position};
});
const canonical=JSON.stringify({schemaVersion:2,topics,cards});
const output={schemaVersion:2,generatedAt:new Date().toISOString(),sourceRevision:hash(canonical),sourceSpreadsheet:"Thai Flash Cards",topics,cards};
